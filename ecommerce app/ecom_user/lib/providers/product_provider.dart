...
class ProductProvider extends ChangeNotifier {
  ...
  getAllCategories() {
    DbHelper.getAllCategories().listen((event) {
      ...
      categoryNameList = List.generate(
          categoryList.length, (index) => categoryList[index].name!);
  ...
  getAllFeaturedProducts() {
    DbHelper.getAllFeaturedProducts().listen((event) {
      featuredProductList = List.generate(event.docs.length,
          (index) => ProductModel.fromMap(event.docs[index].data()));
      notifyListeners();
    });
  }
  ...
  Future<String> updateImage(XFile xFile) async {
    ...
    final photoRef =
        FirebaseStorage.instance.ref().child('UseImages/$imageName');
  ...
  Future<void> addNewRating(double value, String pid) async {
    ...
    final List<RatingModel> ratingList = List.generate(qSnapshot.docs.length,
