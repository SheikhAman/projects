...
class DbHelper {
  ...
  static Future<void> updateCategoryProductCount(
    ...
    for (var cartM in cartList) {
      final catM =
          categoryList.firstWhere((element) => element.name == cartM.category);
      final catDoc = _db.collection(collectionCategory).doc(catM.id);
      wb.update(
          catDoc, {categoryProductCount: catM.productCount - cartM.quantity});
    }
    return wb.commit();
  }
  ...
  static Future<QuerySnapshot<Map<String, dynamic>>> getAllRatingsByProduct(
      String pid) {
    return _db
        .collection(collectionProduct)
        .doc(pid)
